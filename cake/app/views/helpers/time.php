...
//	}
	 ...
	 function niceShort($date_string = null, $sep = ', ')
	 {
    $date = $date_string ? $this->fromString($date_string) : time();
    ...
    if ($this->isToday($date))
    {
      $ret = "Today" . $sep . date("g:ia", $date);
    } 
    elseif ($this->isTomorrow($date))
    {
      $ret = "Tomorrow" . $sep . date("g:ia", $date);
    }
    elseif ($this->isThisWeek($date))
    {
      $ret = date("l" . $sep . "g:ia", $date);
    }
    elseif ($this->wasYesterday($date)) {
      $ret = "Yesterday" . $sep . date("g:ia", $date);
    } else {
      $ret = date("F jS{$y}" . $sep . "g:ia", $date);
    }
 ...
 */
  ...
  function startEnd($start_date, $end_date, $sep = ': ')
  {
    $result = $this->niceShort($start_date, $sep);
    if ($this->sameDay($start_date, $end_date))
    {
      $end = date_parse($end_date);
      $result .= ' - ' . $this->format('g:ia', $end_date);
    }
    else
    {
      $result .= ' - ' . $this->niceShort($end_date, $sep);
    }
    return $this->output($result);
  }
  
  function __same($field, $start_date, $end_date)
  {
    ...
    return ($start_date[$field] == $end_date[$field]);
  }
  
  function sameYear($start_date, $end_date)
  {
    return $this->__same('year', $start_date, $end_date);
  }
  
  function sameMonth($start_date, $end_date)
  {
    return ($this->sameYear($start_date, $end_date) && $this->__same('month', $start_date, $end_date));
  }
  
  function sameDay($start_date, $end_date)
  {
    return ($this->sameMonth($start_date, $end_date) && $this->__same('day', $start_date, $end_date));
  }
}
